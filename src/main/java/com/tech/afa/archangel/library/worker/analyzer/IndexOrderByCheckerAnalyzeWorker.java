...
public class IndexOrderByCheckerAnalyzeWorker implements AnalyzeWorker<SQLRequest> {
    ...
    private static final String ADVICE = "The ORDER BY clause filters on field '%s' which is not indexed on table '%s', potentially causing full table scan.";
    private static final String ADVICE_COMPOSITE = "The ORDER BY clause filters on fields '%s' which is not indexed on table '%s', potentially causing full table scan, maybe using composite index.";
