...
public class IndexWhereCheckerAnalyzeWorker implements AnalyzeWorker<SQLRequest> {
    ...
    private static final String ADVICE = "The WHERE clause filters on field '%s' which is not indexed, potentially causing full table scan";
    private static final String ADVICE_COMPOSITE = "The WHERE clause filters on fields '%s' which is not indexed, potentially causing full table scan, maybe using composite index";
